...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'deepseek' not supported in LuckyClaw, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in LuckyClaw, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.luckyclaw/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.luckyclaw/workspace")
		}
	})
	...
	t.Run("empty config", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Model != "nvidia/nemotron-3-super-120b-a12b:free" {
			t.Errorf("default model should be nvidia/nemotron-3-super-120b-a12b:free, got %q", cfg.Agents.Defaults.Model)
		}
	})
}
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.luckyclaw/workspace"},
...
func TestRunFullMigration(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("loading LuckyClaw config: %v", err)
	}
